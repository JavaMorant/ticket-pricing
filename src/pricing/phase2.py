...
def build(run: phaseio.Run) -> list[Path]:
    ...
    lines += [
        ...
        "`preregistration.md` was FROZEN in commit `1c7196d` (SPEC.md §5.3), so those patterns",
        "are pre-registered and testable in principle. They are absent here because there is",
        "no real data yet: the platform exports are parked, and a seeded fixture has no real",
        "academic calendar in it to find. The calendar is a big real driver of student-event",
        "demand, so the errors below are an **upper bound** — the model is deliberately",
        "fighting with one hand behind its back. Re-run on real data once exports arrive and",
